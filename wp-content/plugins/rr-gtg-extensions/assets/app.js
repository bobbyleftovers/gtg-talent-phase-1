  ...
  _createClass(Header, [{
    ...
    value: function init() {
      ...
      this.navItems.forEach(function (item) {
        var toHash = item.getAttribute('href');

        if (toHash && document.querySelector(toHash)) {
          item.addEventListener('click', function (e) {
            e.preventDefault();
            var section = document.querySelector(toHash);
            Object(_lib_scroll_to__WEBPACK_IMPORTED_MODULE_0__["default"])(section);
          });
        }
      });
      this.checkItems.forEach(function (item) {
        item.addEventListener('click', function (e) {
          var toHash = item.getAttribute('href'); // check if hash is on page

          var exists = document.querySelector(toHash);
...
document.addEventListener('DOMContentLoaded', function () {
  new _js_components_header_js__WEBPACK_IMPORTED_MODULE_0__["default"]();
});
